	...
	void LateUpdate () {
		if (target) {
			...
			if(Input.touchCount > 0 || Input.GetMouseButton(0)){
				...
				transform.rotation = Quaternion.Slerp(transform.rotation, rotation, Time.smoothDeltaTime * 16f);
				transform.position = Vector3.Lerp(transform.position, position, Time.smoothDeltaTime * 16f);
